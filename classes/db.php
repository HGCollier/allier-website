...
class db {
  ...
  public function query ($sql, $params = array ()) {
    # Make sure we're not getting an error from a previous query
    $this->_error = false;
    if ($this->_query = $this->_pdo->prepare ($sql)) {
      ...
      if (count ($params)) {
        foreach ($params as $param) {
          # Sets the ? to the corresponding param
  ...
  private function action ($action, $table, $where = array ()) {
    if (count ($where) == 3) {
      ...
      $field = $where [0];      # Username / Password / First Name
      $operator = $where [1];   # = / > / <
      $value = $where [2];      # Max / Halden / Leo
